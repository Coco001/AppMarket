...
public class DetailAppSafeHolder extends BaseHolder<AppInfo> {
	...
	public View InitView() {
		View view = View.inflate(UIUtils.getContext(), R.layout.layout_detail_safeinfo, null);
			...
			if (safe != null) {
				for (int i = 0; i < 4; i++) {
					if (i < safe.size()) {
						...
						Glide.with(UIUtils.getContext()).load(HttpHelper.URL + "image?name=" + safeInfo.safeUrl).into(ivSafes[i]);
						Glide.with(UIUtils.getContext()).load(HttpHelper.URL + "image?name=" + safeInfo.safeUrl).into(ivDes[i]);
					} else {
